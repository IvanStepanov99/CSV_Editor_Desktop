    ...
    fun averageInterestRate(path: String):BigDecimal{
        ...
        try{
            BufferedReader(FileReader(path)).use { br ->
                ...
                println("Total Weighted Interest: $totalWeighted")
                println("Total Debt: ${sumOfDebt(path)}")
                println("Calculated APR: ${totalWeighted.divide(sumOfDebt(path), 2, RoundingMode.HALF_UP)}")
